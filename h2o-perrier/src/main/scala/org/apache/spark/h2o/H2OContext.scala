...
object H2OContext {
  ...
  def toDataFrame[A <: Product : TypeTag](sc: SparkContext, rdd: RDD[A]) : DataFrame = {
    ...
    val rows = sc.runJob(rdd, perPartition(rdd, key, ftypes) _) // eager, not lazy, evaluation
    val res = new Array[Long](rdd.partitions.size)
    rows.foreach{ case(cidx,nrows) => res(cidx) = nrows }
  ...
  private def perSQLPartition( rdd: SchemaRDD, keystr: String, types: Array[Class[_]] ) ( context: TaskContext, it: Iterator[Row] ): (Int,Long) = {
  ...
  private def perPartition[A]( rdd: RDD[A], keystr:String, types: Array[Class[_]] ) ( context: TaskContext, it: Iterator[A] ): (Int,Long) = {
